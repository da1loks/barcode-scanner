...
[![Stars](https://img.shields.io/github/stars/da1loks/barcode-scanner?style=for-the-badge&logo=github)](https://github.com/da1loks/barcode-scanner/stargazers)
[![Forks](https://img.shields.io/github/forks/da1loks/barcode-scanner?style=for-the-badge&logo=github)](https://github.com/da1loks/barcode-scanner/network/members)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg?style=for-the-badge)](LICENSE)

[![Python](https://img.shields.io/badge/Python-3.10%2B-3776AB?style=flat-square&logo=python&logoColor=white)](https://www.python.org/)
[![FastAPI](https://img.shields.io/badge/FastAPI-API-009688?style=flat-square&logo=fastapi&logoColor=white)](https://fastapi.tiangolo.com/)
[![Docker](https://img.shields.io/badge/Docker-ready-2496ED?style=flat-square&logo=docker&logoColor=white)](#docker-deploy-linux-host--usb-camera)
[![OpenCV](https://img.shields.io/badge/OpenCV-camera-5C3EE8?style=flat-square&logo=opencv&logoColor=white)](https://opencv.org/)
[![ZXing](https://img.shields.io/badge/zxing--cpp-decoder-FF6F00?style=flat-square)](https://github.com/zxing-cpp/zxing-cpp)
[![Platform](https://img.shields.io/badge/Windows%20%7C%20Ubuntu%2022.04-lightgrey?style=flat-square)](#install)
[![WebSocket](https://img.shields.io/badge/WebSocket-live-blueviolet?style=flat-square)](#websocket)

**Live USB webcam barcode scanner** — manipulators pull tasks from the backend, get coordinates over HTTP/WebSocket, then ACK.

<br/>

**[English](#english)** · **[Русский](#русский)** · **[API docs](http://127.0.0.1:8080/docs)**
...
> [!IMPORTANT]
> **Architecture rule:** manipulators know the backend URL. The backend never calls robot IPs or webhooks (pull model).

> [!TIP]
> Quick local try: `pip install -r requirements.txt` → `python scan.py` → point at `test_code.png` → you should see `1234567890`.
...
4. Manipulators create tasks, wait for results, and ACK.
...
- Docker deploy with camera passthrough (Linux host)
...
| **Docker** | headless deploy on Linux |
...
> [!WARNING]
> Log out and back in (or reboot) after `usermod -aG video`, or the camera may stay inaccessible.
...
> [!TIP]
> Default camera index is `1` (often USB). Built-in laptop cam is usually `0`. Try `python scan.py 0` if the wrong camera opens.
...
> [!CAUTION]
> Decorative “barcode-looking” images from the internet often are **not** real encodings and will not scan. Use a real product label or `test_code.png`.
...
| `HEADLESS` | `0` | `1` = no preview window (Docker) |
...
> [!NOTE]
> Often the USB cam is `/dev/video0` (`CAM_INDEX=0`). If it is `video1`, edit `devices:` in `docker-compose.yml` and set `CAM_INDEX=1`.
...
> [!TIP]
> If OpenCV cannot open the camera inside the container, uncomment `privileged: true` in `docker-compose.yml`, confirm the device path, and check the `video` group.
...
> [!IMPORTANT]
> **Правило архитектуры:** манипуляторы знают URL бэкенда. Бэкенд сам **не** ходит на IP роботов и не шлёт webhook’и (pull-модель).

> [!TIP]
> Быстрый локальный тест: `pip install -r requirements.txt` → `python scan.py` → наведи на `test_code.png` → в консоли `1234567890`.
...
4. Манипуляторы создают задачи, ждут результат и делают ACK.
...
- Деплой в Docker с пробросом камеры (Linux-хост)
...
| **Docker** | headless-деплой на Linux |
...
> [!WARNING]
> После `usermod -aG video` нужно выйти из сессии и зайти снова (или перезагрузиться), иначе камера может остаться недоступной.
...
> [!TIP]
> По умолчанию индекс камеры `1` (часто USB). Встроенная камера ноутбука обычно `0`. Если открылась не та — `python scan.py 0`.
...
> [!CAUTION]
> Картинки «похожие на штрихкод» из интернета часто **не** являются настоящим кодом и не сканируются. Бери реальную этикетку или `test_code.png`.
...
| `HEADLESS` | `0` | `1` = без окна превью (Docker) |
...
> [!NOTE]
> Часто USB-камера — `/dev/video0` (`CAM_INDEX=0`). Если `video1` — поправь `devices:` в `docker-compose.yml` и выставь `CAM_INDEX=1`.
...
> [!TIP]
> Если камера внутри контейнера не открывается — раскомментируй `privileged: true` в `docker-compose.yml`, проверь путь устройства и группу `video`.
